...
| Requirement | Status |
|---|---|
| Upload/manage multiple documents (PDF/TXT/MD) | ✅ upload, list, delete |
| Chunk + embed + store in a vector DB | ✅ Qdrant, cosine, batched embeddings |
| Multi-turn chat with context memory | ✅ — see [Conversational memory](#conversational-memory) |
| Grounded answers with source citations | ✅ citations built from retrieved chunks, never LLM output |
| Conversations stored in a DB | ✅ SQLite (`DATABASE_URL` swaps in Postgres unchanged) |
| Clean REST API with error handling | ✅ — see [API reference](#api-reference) |
| Streaming responses *(bonus)* | ✅ SSE, token-by-token |
| Hybrid search / reranking *(bonus)* | ✅ cross-encoder — see [Reranking](#reranking) |
| CI pipeline *(bonus)* | ✅ GitHub Actions — backend tests + frontend lint/typecheck/build |
| Auth *(bonus)* | Not built — see [Known limitations](#known-limitations) |
...
First build downloads and caches a local reranker model (~90MB) alongside the usual image
layers — expect a few minutes on a cold build with no cached layers, seconds after.
...
flowchart TB
    subgraph Ingestion
        ...
        B --> C[BackgroundTask]
        C --> D[Extract text]
        D --> E[Chunk<br/>1024 chars / 128 overlap]
    ...
    subgraph Chat turn
        ...
        L --> M[Embed + retrieve<br/>wider candidate pool]
        M --> M2[Rerank: cross-encoder,<br/>additive over cosine top-k]
        M2 --> N{Any chunk above<br/>score threshold?}
...
| Reranking | local cross-encoder, `ms-marco-MiniLM-L-6-v2` | NVIDIA's hosted rerank NIMs were evaluated first and rejected — see [Reranking](#reranking) |
...
**The core problem: follow-up questions.** A conversational RAG system has one hard
problem that single-turn Q&A doesn't. Given:
...
Two failure modes were found during development and drove design decisions:
...
under [Deliberately not built](#deliberately-not-built) rather than pretended away.
...
- **Hybrid search (BM25 + RRF)** — a sibling project of mine (ContextQuery) implemented this
  ...
  than repeated. Cross-encoder reranking (see [Reranking](#reranking)) is built, and is a
  different technique from hybrid search — it reorders semantic search's own results, rather
  than fusing a second retrieval method's ranking into them.
...
| findability: document referenced by its filename, not its content | this is the exact case reranking regressed — see [Reranking](#reranking) |
...
- No authentication, rate limiting, or multi-tenancy — see [Deliberately not built](#deliberately-not-built).
